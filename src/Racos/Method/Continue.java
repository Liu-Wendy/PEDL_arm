...
public class Continue extends BaseParameters{
	...
	public HashSet<String> occur1;
	public HashSet<String> occur2;
	public int bound;
	...
	protected void Initialize(){
		...
		for(int i=0; i<SampleSize+PositiveNum; i++){
			...
			for(int j=0;j<bound;j++){
				if(temp[i].getFeature(j)==0) break;
				if(temp[i].getFeature(j)==last1) continue;
				path_tmp1+=Integer.toString((int)temp[i].getFeature(j));
				last1=temp[i].getFeature(j);
			}
			occur1.add(path_tmp1);

			double last2=-1;
			String path_tmp2="";
			for(int j=bound;j<2*bound;j++){
				if(temp[i].getFeature(j)==0) break;
				if(temp[i].getFeature(j)==last2) continue;
				path_tmp2+=Integer.toString((int)temp[i].getFeature(j));
				last2=temp[i].getFeature(j);
			}
			occur2.add(path_tmp2);
...
//						pathchoice2.add(NextPop[j].getFeature(1));
						if (notExistInstance(j, NextPop[j])) {
							...
							for(int k=0;k<bound;k++){
								if(NextPop[j].getFeature(k)==0) break;
								if(NextPop[j].getFeature(k)==last1) continue;
								path_tmp1+=Integer.toString((int)NextPop[j].getFeature(k));
								last1=NextPop[j].getFeature(k);
							}
							occur1.add(path_tmp1);

							double last2=-1;
							String path_tmp2="";
							for(int k=bound ;k<2*bound;k++){
								if(NextPop[j].getFeature(k)==0) break;
								if(NextPop[j].getFeature(k)==last2) continue;
								path_tmp2+=Integer.toString((int)NextPop[j].getFeature(k));
								last2=NextPop[j].getFeature(k);
							}
							occur2.add(path_tmp2);
						}
					}
...
//					}
		...
		System.out.println("\nArm1 tries "+occur1.size()+"/21 different paths.");
		System.out.println("Arm2 tries "+occur2.size()+"/21 different paths.");
